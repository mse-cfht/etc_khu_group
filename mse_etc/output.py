...
def display_sn_mag(res_mode, airmass, pwv, exp_t, exp_n, min_mag, max_mag, mag_range, sky, result):
    ...
    if res_mode == "LR":
        ...
        ax = plt.subplot(111)
        ax.plot(mag_range, result[0], 'b', mag_range, result[1], 'g', mag_range, result[2], 'r', linewidth=1)
        ax.plot(mag_range, result[3], 'b', mag_range, result[4], 'g', mag_range, result[5], 'r', linewidth=1, linestyle="dashed")
...
def save_sn_mag(res_mode, airmass, pwv, exp_t, exp_n, min_mag, max_mag, mag_range, sky, result):
    ...
    if res_mode == "LR":
        ...
        f1.write('\n========== Plot data ==========\n')
        f1.write('Point Target Magnitude[ABmag]\tSNR_Blue\tSNR_Green\tSNR_Red\tSNR_Blue(with grating)\t'
                 'SNR_Red(with grating)\tSNR_Red(with grating)\n')
        for i in range(len(mag_range)):
            f1.write('%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\n'
                     % (mag_range[i],result[0][i],result[1][i],result[2][i],result[3][i],result[4][i],result[5][i]))
        ...
        f1.write('\n========== Plot data ===========\n')
        f1.write('Point Target Magnitude[ABmag]\tSNR_Blue\tSNR_Green\tSNR_Red\tSNR_Blue(with grating)\t'
                 'SNR_Green(with grating)\tSNR_Red(with grating)\n')
        for i in range(len(mag_range)):
