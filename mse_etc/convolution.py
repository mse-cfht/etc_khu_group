...
for l in res:
    ...
    for r in range(1, 4):
        ...
        if r == 1:
            hdul.writeto(f'data/skymodel_MSE_pwv1.0_{l}.fits')
        elif r == 2:
            hdul.writeto(f'data/skymodel_MSE_pwv2.5_{l}.fits')
        elif r == 3:
            hdul.writeto(f'data/skymodel_MSE_pwv7.5_{l}.fits')
...
#        data_atmo1[i] = row_red1[i][1]
    for k in res:
        ...
        hdul = fits.HDUList([primary_hdu, table_hdu])
        hdul.writeto(f'data/result_MSE_pwv{l}_{name}_{k}.fits')
...
for k in res:
    ...
    hdul = fits.HDUList([primary_hdu, table_hdu])
    hdul.writeto(f'SKY/MSE_AM1_box_{name}_MR.fits')
